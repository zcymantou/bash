...
Slave_SQL_Running)
   mysql -u${user} -p${passwd} -e "show slave status\G" 2>/dev/null |grep -w Slave_SQL_Running |awk '/Slave_SQL_Running/{print $2}' ;;
user_type)
   #主从判断
   num=`mysql -u${user} -p${passwd} -e "show slave status\G" 2>/dev/null|wc -l`
   if [ $num -gt 3 ];then
      user_type=slave
      echo $user_type
   else
...
log_bin)
    #是否开启二进制
    mysql -u${user} -p${passwd} -e "show variables like 'log_bin'" 2>/dev/null| awk '/log_bin/{print $2}' ;;
